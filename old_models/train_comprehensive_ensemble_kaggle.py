...
try:
    ...
    print("⚠️ CatBoost not found. Please pip install it.")
    pass
...
# Saved features in current working dir
...
if os.path.exists(FEATURE_TRAIN_PATH) and os.path.exists(FEATURE_TEST_PATH):
    ...
    # Robust filter: prefer uncorrected files if available, else take all
    uncorrected_train = [f for f in train_files if 'corrected' not in f]
    if len(uncorrected_train) > 0: train_files = uncorrected_train
    
    uncorrected_test = [f for f in test_files if 'corrected' not in f]
    if len(uncorrected_test) > 0: test_files = uncorrected_test
...
# --- 4. MODELING LOOP ---
print(f"\n🚀 STARTING MULTI-MODEL EXPERIMENT (KAGGLE)")
...
MODELS = {
    'XGB': xgb.XGBClassifier(n_estimators=2000, learning_rate=0.015, max_depth=6, tree_method='hist', eval_metric='aucpr', n_jobs=-1, random_state=42),
    'LGBM': lgb.LGBMClassifier(n_estimators=2000, learning_rate=0.015, max_depth=8, num_leaves=31, n_jobs=-1, random_state=42, verbose=-1),
    'CAT': None
}
try:
    if cb:
        MODELS['CAT'] = cb.CatBoostClassifier(iterations=2000, learning_rate=0.015, depth=6, loss_function='Logloss', eval_metric='F1', verbose=0, random_seed=42)
except:
    pass
...
for k in K_VALUES:
    ...
    oof_preds = {name: np.zeros(len(X)) for name in MODELS if MODELS[name] is not None}
    test_preds = {name: np.zeros(len(X_test_sel)) for name in MODELS if MODELS[name] is not None}
    ...
    for name, model in MODELS.items():
        ...
        for tr_idx, va_idx in kf.split(X_sel, y):
            ...
            if name == 'XGB':
                ...
                clf.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], early_stopping_rounds=150, verbose=False)
            elif name == 'LGBM':
                clf = lgb.LGBMClassifier(**model.get_params())
                clf.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(150, verbose=False)])
            elif name == 'CAT':
        ...
        print(f"-> F1: {best_f1:.4f} (Thresh {best_t:.2f})")
        results.append({'Features': k, 'Model': name, 'F1': best_f1, 'Threshold': best_t})

    # ENSEMBLE
    valid_models = [name for name in MODELS if MODELS[name] is not None]
    if len(valid_models) > 0:
        ens_oof = np.mean([oof_preds[name] for name in valid_models], axis=0)
        ...
        if best_ens_f1 > 0.4:
            ens_test = np.mean([test_preds[name] for name in valid_models], axis=0)
